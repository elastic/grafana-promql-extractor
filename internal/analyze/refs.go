...
// CollectSeries extracts metric and label names referenced in queries so they
// can be seeded with remote write. Label values are placeholders; only presence
// in the mapping matters for coverage analysis.
func CollectSeries(queries []string) []SeriesSpec {
